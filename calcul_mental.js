...
checkboxResults.addEventListener('change', ($event) => {
			if ($event.target.checked) {
				affichageReponse.style.color = 'black';				
			} else {
				affichageReponse.style.color = '#8AEEE7';
			}
		});
		
checkboxResults2.addEventListener('change', ($event) => {
			if ($event.target.checked) {
				affichageReponse2.style.color = 'black';				
			} else {
				affichageReponse2.style.color = '#8AEEE7';
			}
		});
...
function nettoyageAffichage() {
	...
	affichageReponse.textContent = "";
	affichageReponse2.textContent = "";
}
...
function check() {
	...
	if (userGuess === results[count-1]) {
		...
		affichageReponse.textContent += firstNumbers[count-1] + '+' + secondNumbers[count-1] + '=' + results[count-1] + ', ';
		affichageReponse2.textContent += results[count-1] + ', ';
		if (count !== countLimit) {
